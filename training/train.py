...
Updated version:
- Supports both DQN and PPO
- Works with the improved DataDrivenFireEnv observation/state design
- Keeps resume/checkpoint/final GIF logging behavior
"""
import os
from typing import Optional
...
def train(
    ...
    Logs to W&B:
    - Per-episode: reward, episode length
    - Periodic: GIF recordings of agent behaviour
    - Final: evaluation metrics, model checkpoint artifact
    ...
    algo_name: str = str(train_cfg.get("algorithm", "PPO")).upper()
    run_name: str = str(train_cfg.get("run_name", "reference_agent"))
    timesteps: int = int(train_cfg.get("total_timesteps", 50000))
    checkpoint_dir: str = str(train_cfg.get("checkpoint_dir", "checkpoints/"))
    checkpoint_freq: int = int(train_cfg.get("checkpoint_freq", 10000))
    eval_episodes: int = int(train_cfg.get("eval_episodes", 5))
    gif_freq: int = int(train_cfg.get("gif_freq", 10000))
    ...
    try:
        ...
        if algo_name == "DQN":
            ...
            raise ValueError(
                f"Unknown algorithm '{algo_name}'. Supported algorithms: 'DQN', 'PPO'."
            )
        ...
        if resume_from is not None:
            ...
            if remaining <= 0:
                print(
                    f"[train] Already completed {resume_step} steps "
                    f"(target {timesteps}). Nothing to do."
                )
                return
        ...
        # ------------------------------------------------------------------
        # 10. Log final metrics + model artifact to W&B
        # ------------------------------------------------------------------
        logger.log(
            {
                "final_mean_reward": eval_results["mean_reward"],
        ...
        # ------------------------------------------------------------------
        # 11. Record one final GIF using PIL-based custom renderer
        # ------------------------------------------------------------------
        print("[train] Recording final evaluation GIF ...")
        try:
            import shutil
            ...
            gif_env = DataDrivenFireEnv(merged_env_cfg)
            terrain_base = (
                gif_env.build_terrain_base_rgb()
                if hasattr(gif_env, "build_terrain_base_rgb")
                else None
            )
            ...
            while not done:
                ...
                if step_count % max(1, gif_env.num_agents) == 0 or done:
                    ...
                    # Mitigation
                    rgb[fire_map == 3] = (
                        rgb[fire_map == 3] * 0.2 + np.array([0, 100, 255]) * 0.8
                    ).astype(np.uint8)
                    rgb[fire_map == 4] = (
                        rgb[fire_map == 4] * 0.2 + np.array([255, 140, 0]) * 0.8
                    ).astype(np.uint8)
                    rgb[fire_map == 5] = (
                        rgb[fire_map == 5] * 0.2 + np.array([0, 210, 210]) * 0.8
                    ).astype(np.uint8)
